...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  ### START CODE HERE
  # Center_word_vec dimension: (d,)
  # outside_vectors dimension: (V, d)
  # prediction: (v, )
  predictions = np.dot(outside_vectors, center_word_vec) 

  # apply softmax to get prediction distribution
  # y_hat: (v, )
  y_hat = softmax(predictions)

  # get y with dimension (Vocab, ), true outside
  y = np.zeros(y_hat.shape) 
  y[outside_word_idx] = 1 
 
  y_hat_o = y_hat[outside_word_idx]

  # loss = -log(y_hat)_o
  loss = -np.log(y_hat_o)

  # gradient with respect to Vc = U^T(y_hat - y)
  grad_center_vec = np.dot(outside_vectors.T, (y_hat - y))

  # gradient with respect to U = Vc(y_hat - y): (Vocab, nb_of_embedding)
...
def skipgram(current_center_word, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  for outside_word in outside_words:
    ...
    # grad_center: (d, ), grad_outside: (V, d)
    loss_current, grad_center, grad_outside = word2vec_loss_and_gradient(
